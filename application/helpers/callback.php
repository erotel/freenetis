...
class callback
{
	...
	public static function registration_target_type_field($x)
	{
		// 1) když přijde celý řádek
		if (is_object($x)) {
			// nejpravděpodobnější
			if (isset($x->registration_target_type)) {
				$v = (int)$x->registration_target_type;
			}
			// fallbacky kdyby to bylo pod jiným jménem (např. RTT)
			elseif (isset($x->RTT)) {
				$v = (int)$x->RTT;
			}
			// poslední fallback: nic nenalezeno
			else {
				return isset($x->registration_target_type) ? (string)$x->registration_target_type : '';
			}
		} else {
			// 2) když přijde hodnota
			$v = (int)$x;
		}

		if ($v === 90) return __('Member');
		if ($v === 2)  return __('Customer');
		return (string)$v;
	}
	...
	public static function additional_contacts_type_callback($item, $name)
	{
		...
		if (
			isset($item->type) && $item->type == Contact_Model::TYPE_EMAIL &&
			isset($item->user_id)
		) {
	...
	public static function connected_to_device($item, $name)
	{
		if ($item->connected_to_device_id) {
			if ($item->connected_to_devices_count == 1) {
				...
				Controller::instance()->acl_check_view('Links_Controller', 'link') &&
				isset($item->link_id)
			) {
				echo html::anchor(
	...
	public static function device_connected_to_device($item, $name)
	{
		...
		if (
			isset($connected->connected_to_device_id) &&
		...
		if ($message instanceof Message_Model && $message && $message->id) {
			switch ($message->type) {
				case Message_Model::BIG_DEBTOR_MESSAGE:
					...
					if (
						$item->balance < Settings::get('big_debtor_boundary')
						&& !$item->interrupt
			...
			if (
				...
				switch ($name) {
					case 'redirection':
						if (
							isset($item->notification_by_redirection) &&
					...
					case 'email':
						if (
							isset($item->notification_by_email) &&
					...
					case 'sms':
						if (
							isset($item->notification_by_sms) &&
	...
	public static function phone_number_field($item, $name)
	{
		...
		if ($pucontact_id > 0) {
			...
			if ($uid == 0) { // number is not in database => display it and allow to add
			...
			} else { // number bellow to user of free net is, display his name
				$user->find($uid);
	...
	{
		$number = VoIP_calls_Controller::parse_number(
			substr($item->callcon, 4, strlen($item->callcon) - 4),
			$item->area
		);
	...
	public static function device_active_link_url_prepare($url_pattern, $device_id)
	{
		...
		if (
			$controller->acl_check_view('Devices_Controller', 'login') &&
			$dm->login
		); {
		...
		if (
			$controller->acl_check_view('Devices_Controller', 'password') &&
			$dm->password
		); {
