...
class Bank_transfers_Controller extends Controller
{
		...
		if ($ba->member_id == 1) {
			if ($this->acl_check_new('Accounts_Controller', 'bank_transfers')) {
				...
				$grid->add_new_button(
					...
					array(),
					help::hint('add_new_bank_transfer')
				);
				// add new bank fee
				$grid->add_new_button(
					...
					array(),
					help::hint('add_new_bank_transfer_without_counteraccount')
				);
			}
	...
	public function show_by_bank_statement(
		...
		$breadcrumbs = breadcrumbs::add()
			->link('bank_accounts/show_all', 'Bank accounts')
			->link(
				'bank_statements/show_by_bank_account/' . $bs->bank_account_id,
				'Bank statements'
			)
			->disable_translation()
			->text($headline)
	...
	public function add($baa_id = null)
	{
		...
		$breadcrumbs = breadcrumbs::add()
			...
			->link(
				'bank_transfers/show_by_bank_account/' . $baa_id,
				$baa->name . ' (' . $baa->id . ')'
			)
			->text('Bank transfers')
			->disable_translation()
		...
		if ($form->validate()) {
			...
			try {
				...
				if ($form_data['type'] == self::$bank_fee) {
					...
					$form_data['type'] == self::$bank_interest ||
