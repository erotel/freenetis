...
class Scheduler_Controller extends Controller
{
			...
			if (($this->t - $last_active) > 110) // diff higher than 110 seconds => log failure
			{
				...
				$desc = __(
					'CRON (scheduler) was offline from %s to %s (%s) some ' .
						'system operation may not be performed',
					array(
						$data['from'],
						$data['to'],
			...
			foreach ($accounts as $account) {
				...
				try {
					...
					if ($bs) {
						...
						$m = __(
							'Bank statement (%s) with %d transfers for bank account '
				...
				foreach ($messages as $message) {
					...
					try {
						...
						if (count($info_messages)) {
							$m = __(
								'Notification message "%s" has been automatically activated',
								__($message->name)
							);
	...
	private function send_message_activation_report(
		$to_emails,
		$description,
		$actions,
		$activated_members
	...
	private function fee_deduction()
	{
		...
		if (
			Settings::get('deduct_fees_automatically_enabled') &&
	...
	{
		if (
			date('H:i', $this->t) == '00:' . self::AM_INTERRUPTED_MEMBER &&
			...
			try {
				...
				$end_interr_members = $m_model->get_interrupted_members_on(
					date('Y-m-d', strtotime('-1 day', $this->t)),
					3
				);
	...
	private function monitoring_notification()
	{
		if (module::e('monitoring') && (
			...
			try {
				...
				$monitoring_host_down_message = $message_model->where('type', Message_Model::MONITORING_HOST_DOWN)->find();
				...
				$monitoring_host_up_message = $message_model->where('type', Message_Model::MONITORING_HOST_UP)->find();
	...
	{
		if (
			Settings::get('vtiger_integration') &&
