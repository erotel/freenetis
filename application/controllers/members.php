...
class Members_Controller extends Controller
{
	...
	public function show_all(
		...
		if ($regs) {
			...
			$grid->form_extra_buttons = array(
				form::hidden(
					'url',
					url_lang::current() . server::query_string()
				)
			);
		} else {
		...
		foreach ($selected as $applicant_id) {
			...
			try {
				...
				if (!$group_aro_map->exist_row(
					Aro_group_Model::REGULAR_MEMBERS,
					$main_user_id
				)) {
	...
	public function approve_applicant($member_id = NULL)
	{
		...
		if (Settings::get('finance_enabled')) {
			...
			$additional_payment_amount = $member_fee->calculate_additional_payment_of_applicant(
				$member->applicant_connected_from,
				date('Y-m-d')
			);
		}
		...
		// access control
		if (
			!$this->acl_check_new(get_class($this), 'members') ||
		...
		if ($form->validate()) {
			try {
				...
				if (
					isset($form_data['allow_additional_payment']) &&
					...
					Transfer_Model::insert_transfer(
						$credit_account->id,
						$operating_account->id,
						null,
						null,
						$this->session->get('user_id'),
						null,
						$member->entrance_date,
						date('Y-m-d H:i:s'),
	...
	public function show(
		...
		if (
			$member->address_point_id &&
			$member->address_point->id
		) {
			$address = '';

			if (
				$member->address_point->street_id &&
			...
			if (
				$member->address_point->country_id &&
		...
		if ($member->members_domicile->address_point->id) {
			...
			if (
				$member->members_domicile->address_point->street_id &&
			...
			if (
				$member->members_domicile->address_point->town_id &&
			...
			if (
				$member->members_domicile->address_point->country_id &&
		...
		if (
			$this->acl_check_new('Users_Controller', 'users') ||
		...
		if (
			$this->acl_check_edit('Users_Controller', 'users') ||
			($this->session->get('user_type') == User_Model::MAIN_USER &&
		...
		if (Settings::get('redirection_enabled')) {
			...
			if (
				$this->acl_check_new('Redirect_Controller', 'redirect') &&
				$total_ips < 1000
			) // limited count
			{
				$redir_grid->add_new_button(
					'redirect/activate_to_member/' . $member_id,
					__('Activate redirection to member'),
					array(),
		...
		if ($is_former) {
			if (
				$this->acl_check_delete(get_class($this), 'members')
		...
		if (Settings::get('finance_enabled')) {
			...
			if (
				...
				$member_links[] = html::anchor(
					'transfers/show_by_account/' . $account->id,
					__('Show transfers')
				);
			}
			...
			if ($this->acl_check_view(get_class($this), 'fees', $member->id)) {
				$member_links[] = html::anchor(
					'members_fees/show_by_member/' . $member->id,
					__('Show tariffs')
				);
			}
		}
		...
		if (!$is_association) {
			if (!$is_former) {
				...
				if (
					Settings::get('allowed_subnets_enabled') &&
			...
			if (module::e('notification')) {
				if ($this->acl_check_new('Notifications_Controller', 'member')) {
					$member_links[] = html::anchor(
						'notifications/member/' . $member->id,
						__('Notifications'),
				...
				if ($this->acl_check_view('Members_whitelists_Controller', 'whitelist')) {
					$member_links[] = html::anchor(
						'members_whitelists/show_by_member/' . $member->id,
						__('Whitelists')
					);
				}
			}
		...
		if (!$is_former) {
			...
			if (
				$this->acl_check_edit('Users_Controller', 'password', $member->id) &&
				...
				try {
					...
					if ($form_data['use_domicile']) {
						...
						if ($member->address_point_id != $address_point->id) {
							// add GPS
							if (!empty($domicile_gpsx) && !empty($domicile_gpsy)) { // save
								$address_point->update_gps_coordinates(
									$address_point->id,
									$domicile_gpsx,
									$domicile_gpsy
	...
	public function edit($member_id = NULL)
	{
		...
		if (
			$this->acl_check_edit(get_class($this), 'type', $member->id) &&
		...
		if (
			$this->acl_check_edit(get_class($this), 'qos_ceil', $member->id) &&
		...
		if (
			$this->acl_check_edit(get_class($this), 'entrance_date', $member->id) &&
			$member->type != Member_Model::TYPE_APPLICANT
		) {
			$form->date('entrance_date')
				->label('Entrance date')
	...
	private function registration_html_export($member_id, $page_width = '18cm')
	{
		...
		foreach ($contacts as $contact) {
			if ($contact->type == Contact_Model::TYPE_PHONE) {
				...
				$contact->type == Contact_Model::TYPE_ICQ ||
...
*/
	...
	public function valid_unique_email($input = NULL)
	{
		...
		if (
			$value && !Settings::get('user_email_duplicities_enabled') &&
	...
	public function valid_member_type($input = NULL)
	{
		...
		if (
			$this->input->post('end_membership') &&
		...
		if ($login) {
			...
			if (is_array($accounts_desc)) {
				...
				foreach ($accounts_desc['fields'] as $desc) {
					...
					if (
						$desc['mandatory'] == 1 && !in_array($desc['name'], $account_fields) &&
			...
			if (is_array($contacts_desc)) {
				...
				foreach ($contacts_desc['fields'] as $desc) {
					...
					//check mandatory fields
					if (
						$desc['mandatory'] == 1 && !in_array($desc['name'], $contact_fields) &&
						$desc['name'] != 'assigned_user_id'
					)
						throw new Exception(__('Error - wrong vtiger fields settings'));
				}
			...
			if (is_array($result)) {
				...
				if (!empty($accounts_create_ids)) {
					...
					foreach ($accounts_create as $account) {
						...
						$acc_arr[$account_fields['street']] = address::street_join(
							$account->street,
			...
			if (is_array($result) && isset($links)) {
				...
				if (!empty($contacts_create_ids)) {
					...
					foreach ($contacts_create as $contact) {
						...
						$con_arr[$contact_fields['street']] = address::street_join(
							$contact->street,
			...
			if ($accounts_update->count() != 0) {
				...
				if (is_array($results)) {
					...
					foreach ($accounts_update as $record) {
						...
						if (
							$record->entrance_date != $record_vtiger[$account_fields['entrance_date']] &&
			...
			if ($contacts_update->count() != 0) {
				...
				if (is_array($results)) {
					...
					foreach ($contacts_update as $record) {
						...
						if (
							$record->birthday != $record_vtiger[$contact_fields['birthday']] &&
