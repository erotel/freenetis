...
class Invoices_Controller extends Controller
{
				...
				if ($form->validate()) {
					...
					if ($invoice_template->type != Invoice_template_Model::TYPE_DBASE) {
						...
						foreach ($xpath->evaluate($invoice_template->invoices) as $xpath_invoice) {
							...
							$invoice_type = @$xpath->evaluate(
								$invoice_template->invoice_type,
								$xpath_invoice
							);
							...
							$partner_org_id = @$xpath->evaluate(
								$invoice_template->sup_organization_identifier,
								$xpath_invoice
							);
							...
							if ($invoice_type === false) {
								...
								$customer_org_id = @$xpath->evaluate(
									$invoice_template->cus_organization_identifier,
									$xpath_invoice
								);
						...
						foreach ($records as $record) {
							...
							if (!empty($partner_id))
								...
								if (
									array_key_exists($fields['account_nr'], $record) &&
							...
							if (
								array_key_exists($fields['currency'], $record) &&
	...
	public function export_filter($invoice_template_id = NULL)
	{
		...
		foreach ($invoice_templates as $invoice_template)
			if (
				$invoice_template->type != Invoice_template_Model::TYPE_ED_INV &&
	...
	public function edit($invoice_id = NULL)
	{
		...
		$this->form->input('order_nr')
			...
			->rules('length[3,40]|valid_numeric')
